...
export const createAppointment = async (req, res, next) => {
    try {
        const { firstName, lastName, email, mobNumber, NIC, dob, gender, department, appointmentDate, appointmentTime, address, notes } = req.body;
...
export const updateAppointmentStatus = async (req, res, next) => {
    try {
        ...
        const appointment = await Appointment.findByIdAndUpdate(id, { status, notes, updatedAt: new Date() }, { new: true });
        if (!appointment) {
...
export const assignDoctorToAppointment = async (req, res, next) => {
    try {
        ...
        const appointment = await Appointment.findByIdAndUpdate(id, { doctorId, updatedAt: new Date() }, { new: true }).populate('doctorId', 'firstName lastName specialty');
        if (!appointment) {
            res.status(404).json({ message: "Appointment not found" });
            return;
        }
