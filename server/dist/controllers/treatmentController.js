...
export const createTreatment = async (req, res, next) => {
    try {
        ...
        const { patientId, doctorId, appointmentId, treatmentName, description, diagnosis, medications, procedures, notes, startDate, endDate, status, followUpDate, } = req.body;
        ...
        await createNotification(doctorId, 'doctor', 'treatment_schedule', `New treatment scheduled: ${treatmentName} for patient ${patientId}`, treatment.id);
...
export const updateTreatment = async (req, res, next) => {
    try {
        ...
        const treatment = await Treatment.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true })
            .populate("patientId", "firstName lastName email")
            .populate("doctorId", "firstName lastName specialty");
        if (!treatment) {
        ...
        if (oldTreatment && oldTreatment.status !== 'ongoing' && treatment.status === 'ongoing') {
            await createNotification(treatment.patientId.toString(), 'patient', 'treatment_approved', `Your treatment "${treatment.treatmentName}" has been approved and is now ongoing.`, treatment.id);
        }
